...
struct FWaterUtils
{
	...
	* @param InAdditionalObjectFlags is the additional flags that should be set on the newly-created object, if any (it will come out with at least RF_Transient as the name of the function implies)
	 ...
	 */
	static CUSTOMWATER_API UMaterialInstanceDynamic* GetOrCreateTransientMID(UMaterialInstanceDynamic* InMID, FName InMIDName, UMaterialInterface* InMaterialInterface, EObjectFlags InAdditionalObjectFlags = RF_NoFlags);
	 ...
	 */
	static CUSTOMWATER_API UTextureRenderTarget2D* GetOrCreateTransientRenderTarget2D(UTextureRenderTarget2D* InRenderTarget, FName InRenderTargetName, const FIntPoint& InSize, ETextureRenderTargetFormat InFormat, 
	...
	static CUSTOMWATER_API bool IsWaterEnabled(bool bIsRenderThread);
	static CUSTOMWATER_API bool IsWaterMeshEnabled(bool bIsRenderThread);
	static CUSTOMWATER_API bool IsWaterMeshRenderingEnabled(bool bIsRenderThread);
	static CUSTOMWATER_API float GetWaterMaxFlowVelocity(bool bIsRenderThread);
